...
use alloc::vec::Vec;
use ark_bn254::{Fq, G1Projective, G1Affine};
use ark_ec::VariableBaseMSM;
use ark_serialize::{CanonicalDeserialize, CanonicalSerializeHashExt};
...
impl PedersenGens {
    ...
    pub fn commit(&self, value: Fq, blinding: Fq) -> G1Projective {
        VariableBaseMSM::msm(&[self.B, self.B_blinding], &[value, blinding])
    }
}
...
impl Default for PedersenGens {
    fn default() -> Self {
        PedersenGens {
            B: G1Affine::identity(),
            B_blinding: G1Affine::identity().hash(),
        }
    }
}
...
impl Iterator for GeneratorsChain {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(G1Projective::deserialize_compressed(&uniform_bytes).unwrap())
    }
