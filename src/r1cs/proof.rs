...
impl R1CSProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof, R1CSError> {
        ...
        let t_x = Fq::from(read32!()).ok_or(R1CSError::FormatError)?;
        let t_x_blinding = Fq::from(read32!()).ok_or(R1CSError::FormatError)?;
        let e_blinding = Fq::from(read32!()).ok_or(R1CSError::FormatError)?;
